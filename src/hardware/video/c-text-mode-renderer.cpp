...
CTextModeRenderer::CTextModeRenderer(CMemory* main, CMemory* aux, CTextModeUnit* textModeUnit, CGraphicModeUnit* graphicModeUnit, CVideoOutput* vo)
    ...
    this->primaryFont = new Font(":roms/char.rom");
    this->alternativeFont = new Font(":roms/char2.rom");
}
...
{
//    qDebug("CTextModeRenderer::render(%d,%d)",startline,endline);
...
QImage &CTextModeRenderer::renderToBitmap(QImage &bitmap, int startline, int endline)
{
    //    qDebug("CTextModeRenderer::render(%d,%d)",startline,endline);
    CTextRenderer *renderer;
    ...
    if (!textModeUnit) {
        ...
        renderer->set_charset((textModeUnit->get_altchar())?alternativeCharset:primaryCharset);
        renderer->set_font((textModeUnit->get_altchar())?alternativeFont:primaryFont);
    }
    return renderer->renderToBitmap(bitmap, startline, endline);
}
